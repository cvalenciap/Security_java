...
(function ($) {
  ...
  $.fn.serializeJSON = function (options) {
    ...
    f.readCheckboxIndeterminateValues(formAsArray, this, opts); //método adicionado para el soporte del estado indeterminado de los checkbox
  ...
  $.serializeJSON = {
    ...
    defaultOptions: {
      ...
      checkboxIndeterminateValue: undefined, //para incluir checkbox indeterminados
    ...
    setupOpts: function(options) {
      ...
      validOpts = ['checkboxIndeterminateValue', 'checkboxUncheckedValue', 'parseNumbers', 'parseBooleans', 'parseNulls', 'parseAll', 'parseWithFunction', 'customTypes', 'defaultTypes', 'useIntKeysAsArrayIndex']; // re-define because the user may override the defaultOptions
      for (opt in options) {
      ...
      return {
        ...
        checkboxIndeterminateValue: optWithDefault('checkboxIndeterminateValue'),
    ...
    //Autor: Carlos Altamirano Briceño
    //Se adiciona el método para detectar el estado indeterminado del checkbox
    //para poder ser usado en la ventana de opciones del módulo de seguridad
    readCheckboxIndeterminateValues: function (formAsArray, $form, opts) {
        if(navigator.appVersion.indexOf("MSIE 8.")==-1){
            var selector, $indeterminateCheckboxes, $el, dataUncheckedValue, f;
            if (opts == null) { opts = {}; }
            f = $.serializeJSON;

            selector = 'input[type=checkbox][name]:indeterminate:not([disabled])';
            $indeterminateCheckboxes = $form.find(selector).add($form.filter(selector));
            $indeterminateCheckboxes.each(function (i, el) {
                $el = $(el);
                if (!f.isUndefined(opts.checkboxIndeterminateValue)) {
                    formAsArray.push({name: el.name, value: el.value});
                }
            }); 
        }
      },
